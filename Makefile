...
.PHONY: help phases phase 0 1 2 3 4 000 001 002 003 004 005 006 100 101 102 103 104 105 106 107 108 200 201 202 203 204 205 206 207 208 209 210 211 212 213 214 300 400 list \
...
phase: ## run a learning phase alias, e.g. `make phase 002`
	@case "$(PHASE_ARG)" in \
	  ...
	  4|400) $(MAKE) --no-print-directory -C $(PHASE4) phase "$(PHASE_ARG)" ATTACHED="$(ATTACHED)" ;; \
...
0 1 2 3 4 000 001 002 003 004 005 006 100 101 102 103 104 105 106 107 108 200 201 202 203 204 205 206 207 208 209 210 211 212 213 214 300 400 list:
	@:
...
doctor: ## common health check; not a phase step
	...
	@missing=0; \
	for c in qemu-system-x86_64 losetup findmnt sgdisk partprobe mkfs.fat mkfs.ext4 mkfs.xfs mount umount chroot modprobe depmod truncate tar blkid bootctl cpio curl gzip sha256sum sudo ssh ssh-keygen visudo mdbook; do \
