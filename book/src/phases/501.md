...
The distinction matters: moss does not care about `base/` vs `core/` vs `services/`.
As far as moss is concerned there is one flat namespace of `.stone` files. The three
...
the strictest Rust-first, static-musl bar; a `base/` data package is judged mostly on
runtime cleanliness; a `services/` daemon carries the extra burden of unit-file
hygiene. Grouping the packages this way puts the reviewer in the right frame of mind.
...
- **Target triple / C runtime / Link model / Shared runtime libraries** — the
  musl-only audit. `Link model` is `static musl`, `static-pie musl`, or the explicit
  `dynamic musl exception`; `Shared runtime libraries` is `none` or a documented
  exception. This is where accidental glibc or a stray shared-library dependency gets
  caught on paper.
