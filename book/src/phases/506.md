...
Bumping `release: 1 -> 2` gives the new payload a distinct identity, so caches,
indexes, and rollback history can never confuse the old collision-prone build with the
fixed one. This is basic distro hygiene, and Phase 506 practices it locally so it is
already habit by the time a real repo is serving upgrades.
...
- the canonical local repo installs in strict ownership mode.

Strict ownership mode means:

```text
Moss duplicate-entry messages are fatal
```

Phase 505 allowed the warning so we could finish assembling the repo and record
the discovery.

Phase 506 makes that same class of warning unacceptable.
...
3. reassembles the canonical local repo in strict ownership mode.
