...
**Background: `switch_root`.** `switch_root` is the moment the initramfs
`/init` hands the machine over to the real operating system. It makes the
freshly-mounted ONIX root the new `/`, frees the RAM-based initramfs, and `exec`s
PID 1 (`/usr/lib/systemd/systemd`) from the real disk. The catch is that the
initramfs and everything in it — including its bundled copy of `modprobe` and the
module tree — is *gone* the instant this happens. So any module the running
system wants *after* switch-root (for example `vfat` to mount `/boot`) must be
loadable from the real root, not from the initramfs that no longer exists. That
is the gap Phase 214 fills.
...
**Background: `depmod` and `modules.dep`.** Modules depend on each other —
loading `vfat` first needs `fat`, which needs the NLS character-set modules.
`modprobe` does not rediscover those relationships every time; it reads a
precomputed index, `modules.dep`, that lists each module and the modules it
requires. The tool that *builds* that index by scanning a module tree is
`depmod`. Because Phase 214 just rewrote the tree (decompressing every `.ko.gz`
to a plain `.ko`, which changes file names and contents), the old index is stale
and would point at files that no longer exist. So the script runs `depmod -b
"$MNT" "$kernel_release"` to regenerate `modules.dep` against the modules that
are actually present. Skip this and `modprobe` would look up `vfat`, consult a
dependency map that still names `vfat.ko.gz`, and fail to find it.
...
**Background: why the module tree is keyed by kernel release.** A `.ko` file is
compiled against one specific kernel and refuses to load into a different one —
the kernel checks a "vermagic" string and rejects mismatches. That is why module
trees live under `/usr/lib/modules/<kernel-release>/` (here
`6.18.38-0-virt`): the directory name *is* the kernel version, so `modprobe` can
find the set that matches the running kernel and only that set. Phase 214
discovers the release directly from the initramfs's module tree and refuses to
proceed if there is anything other than exactly one release directory, so it can
never staple modules from the wrong kernel onto the image.
