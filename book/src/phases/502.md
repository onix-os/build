...
```text
static or self-contained musl only
...
But for core packages, static or static-pie musl is preferred.
...
That should be a documented exception in `PACKAGE.md`.

Default acceptance should remain static/self-contained.
...
nobody noticed. This is the same reasoning that made `onix-systemd` carry an explicit
documented exception rather than quietly slipping its shared libraries past the gate.
...
binary static" or "stop shipping that wrapper script."
