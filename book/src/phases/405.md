...
### Background: why forwarding is needed at all

With user-mode networking (see Phase 404), the guest lives behind QEMU's simulated
NAT router on the private address `10.0.2.15`. That address is only meaningful
*inside* QEMU's little network — nothing on the host can route packets to it, the
same way nothing on the public internet can reach a machine sitting behind your
home router's NAT. Connections naturally flow guest-to-host (outbound); the
reverse direction has to be arranged deliberately.

**Port forwarding** is that arrangement. `hostfwd=tcp:127.0.0.1:7665-:6649` tells
QEMU: "listen on the host's loopback at TCP port 7665; whenever something
connects, open a matching connection to the guest's port 6649 and splice the two
together." So a byte the host writes to `127.0.0.1:7665` pops out inside the guest
as if it had arrived on `10.0.2.15:6649`. The full path the proof exercises is:

```text
host nc -> 127.0.0.1:7665 -> QEMU hostfwd -> guest 10.0.2.15:6649 -> guest listener
```

The listener end is BusyBox `nc` (netcat), a minimal tool that either dials a TCP
connection or, in `-l` listen mode, accepts one and hands the socket to a program.
Proving this plumbing end-to-end is the prerequisite for SSH: SSH is, at bottom,
just a program listening on a forwarded TCP port — with authentication and
encryption layered on top, which is what the next phase adds.
