...
| --- | --- | ---: | --- | --- |
| `onix-branding` | `base` | 101 | canonical | ONIX identity, `/etc/issue`, MOTD, logo assets |
| `onix-filesystem` | `base` | 102 | canonical | base filesystem labels/defaults and profile templates |
| `onix-busybox` | `core` | 409/506/513 | bootstrap canonical | static musl bootstrap/recovery command set; common coreutils command ownership moved to uutils |
| `onix-dropbear` | `services` | 412 | bootstrap canonical | static musl SSH daemon for early machine access |
| `onix-systemd` | `services` | 422 | canonical | native musl systemd PID 1 and system-management tools |
| `onix-bootstrap-policy` | `services` | 418 | bootstrap canonical | bootstrap service and networking policy |
...
| `onix-rootasrole-policy` | `services` | 512 | bootstrap privilege policy | first RootAsRole factory policy under `/usr/share/factory/etc`; root actor only |
