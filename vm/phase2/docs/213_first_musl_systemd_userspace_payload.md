...
## Background: the switch-root handoff and "userspace"

It helps to slow down the exact moment Phase 212 failed. The initramfs (from
Phase 211) runs in RAM, mounts the real ONIX root by its label, and then performs
a **switch-root**: it makes the newly-mounted root filesystem *the* root, throws
away the RAM filesystem it came from, and `exec`s the program named by
`init=` on the kernel command line. From that instant, the machine is running
from the real disk and the first process — PID 1 — *is* whatever lives at
`/usr/lib/systemd/systemd`. "Userspace" is simply everything that is not the
kernel: the ordinary programs and libraries the kernel runs. So "systemd
userspace" means the systemd program and every file it needs to actually
function.

In Phase 212 the switch-root succeeded but the `exec` failed, because
`/usr/lib/systemd/systemd` was an empty promise — the kernel handed control to an
address with nothing there. Phase 213's entire job is to put a *working* musl
systemd at that address, so the handoff can complete.

## Background: why "userspace", not "a binary"

A naive fix would be "copy the systemd executable to that path." That is not
enough, for two independent reasons.

First, systemd is a **suite, not a program**. PID 1 immediately wants helpers
(`systemd-udevd` to manage devices), unit files (`multi-user.target` and its
dependencies) that describe what to start, and command tools (`systemctl`,
`journalctl`). Miss those and systemd starts but has nothing coherent to do.

Second, the systemd binary is **dynamically linked**. A dynamically linked
program does not contain the code for `printf`, `malloc`, or mounting a
filesystem; instead it records the *paths* of a **dynamic loader** (the
`ld-musl-*.so.1` interpreter) and a set of **shared libraries** (`.so` files),
and asks the kernel to load them at startup. If any of those referenced paths is
missing, the program cannot even begin — the kernel reports "no such file or
directory" for a file that plainly exists, because the file it is really
complaining about is the *loader*. The sections below ("Why copying a Nix closure
is different from copying one binary") walk exactly this problem, which is why
Phase 213 stages a whole runtime **closure** rather than a single file.
...
A Nix **closure** is the transitive set of `/nix/store` paths a thing depends on:
the package, its libraries, their libraries, the loader, and so on, all the way
down until nothing new is referenced. Because Nix records every dependency by
hash, it can compute this set exactly — nothing is "probably needed"; the closure
*is* the complete runtime. Copying the closure guarantees that every path the
systemd binary might resolve at runtime is present on the disk.
