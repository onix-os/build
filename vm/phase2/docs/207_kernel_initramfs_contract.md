...
> **The chicken-and-egg the initramfs solves.** To mount the real root, the
> kernel needs the driver for the disk it lives on and the code for that root's
> filesystem type. But those live *on* the root — which is not mounted yet. The
> initramfs breaks the loop: it is a tiny root the kernel can unpack straight into
> RAM, carrying just enough (disk drivers, filesystem support, a tool to resolve
> `LABEL=`) to find and mount the real root, then hand control over to it. Once the
> real `/` is mounted, the initramfs has done its job and is discarded.

> **Kernel modules vs built-in.** A kernel can either compile a driver *into* the
> kernel image or ship it as a loadable *module* (a `.ko` file under
> `/usr/lib/modules/<version>/`). Modular kernels are smaller but push the problem
> to early boot: any module needed to reach the root — the disk driver, the
> filesystem — must be present *inside the initramfs*, because that is the only
> place they can be loaded from before `/` exists. This is exactly why step 214
> exists: it stages a matching kmod/module payload so the switched-root system can
> later load modules like `vfat`.
...
> **Why `virtio` specifically.** ONIX's first boot target is QEMU, not real
> hardware. QEMU presents its virtual disk and PCI bus through the **virtio**
> family of paravirtualized drivers — `virtio_pci` for the bus and `virtio_blk`
> for the block device. A kernel/initramfs with no virtio support would see *no
> disk at all* under QEMU and panic immediately. On real hardware later, the
> equivalent would be the NVMe/AHCI/SATA drivers instead; the contract names
> virtio because that is what the Phase 212 probe boots on.
...
> **But Phase 2 does borrow a kernel — isn't that a contradiction?** No. Phase 2's
> boot probe (step 211) copies the Alpine forge's virt kernel/initramfs into the
> image *as an explicitly labelled, temporary scaffold* so the disk-and-userspace
> plumbing can be proven now. Phase 207's decision is about the *final* ownership:
> the shipped ONIX kernel must come from ONIX-owned `onix-kernel`/`onix-initramfs`
> stones, and the reserved Phase 3 replaces the borrowed payload with them.
> Borrowing to prove a layout is fine; *shipping* someone else's kernel as ONIX's
> own is what the contract forbids. This mirrors how Alpine itself is a throwaway
> forge — its tooling helps, none of its artifacts are meant to survive into ONIX.
