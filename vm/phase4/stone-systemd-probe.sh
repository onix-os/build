#!/usr/bin/env bash
# vm/phase4/stone-systemd-probe.sh — Phase 417 live onix-systemd proof.
#
# Phase 416 installed onix-systemd into the image and materialized its bundled
...
compgen -G "$ONIX_ROOT/artifacts/onix-local-repo/onix-systemd-*.stone" >/dev/null \
  || die "missing onix-systemd stone in artifacts/onix-local-repo (run make phase 415)"

log "Phase 417 stone systemd live proof"
log "goal      : boot with onix-systemd materialized as PID 1 runtime payload"
...
ONIX_SSH_SUCCESS_MESSAGE="Phase 417 proved the image boots with onix-systemd materialized as the PID 1 runtime payload." \
  "$SCRIPT_DIR/ssh-probe.sh" "${probe_args[@]}"

cat <<EOF

==> success
Phase 417 proved the booted ONIX image can use onix-systemd for the active
