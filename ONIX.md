...
> universe contains a musl-targeted `pkgsMusl.systemd`, so the current project
> path is systemd-on-musl. Phase 0's Alpine OpenRC/GRUB setup remains throwaway
...
| Shell hooks | `/usr/share/defaults/etc/profile.d/onix-nix.sh` | PATH, `NIX_REMOTE=daemon` default, `XDG_DATA_DIRS`, `LOCALE_ARCHIVE` guard, fontconfig include. |
